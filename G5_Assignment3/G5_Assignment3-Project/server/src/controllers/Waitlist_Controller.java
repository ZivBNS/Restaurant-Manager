...
public class Waitlist_Controller {
    ...
    private static Message CancelWitlistAndReservationByCode(Message msg) {
        ...
        if (r == null || r.getStatus().equalsIgnoreCase("Completed") || r.getStatus().equalsIgnoreCase("Canceled")) {
            return new Message(MessageType.RESERVATION_CANCEL_FAILED, confirmationCode);
        }

        boolean waitlistExists = false;
        boolean waitlistDeletedSuccessfully = false;

        Waitlist w = WaitlistRepository.getByReservationId(r.getId());
        
        if (w != null) {
            waitlistExists = true;
            waitlistDeletedSuccessfully = WaitlistRepository.deleteById(w.getId());
            
            if (!waitlistDeletedSuccessfully) {
                return new Message(MessageType.WAITLIST_CANCELED_FAILED, confirmationCode);
            }
        }
        ...
        if (reservationCanceled) {
            if (waitlistExists) {
                return new Message(MessageType.WAITLIST_CANCELED, confirmationCode);
            }
            return new Message(MessageType.RESERVATION_CANCELED, confirmationCode);
        }
        return new Message(MessageType.RESERVATION_CANCEL_FAILED, confirmationCode);
    }
}