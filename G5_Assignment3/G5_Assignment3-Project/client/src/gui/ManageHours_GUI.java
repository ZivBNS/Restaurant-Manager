...
/**
 * Controller for the Opening Hours Management screen. Provides a GUI for batch
 * updating weekly schedules and managing special exceptions.
 */
public class ManageHours_GUI {
	...
	@FXML
	private VBox vboxRegularHours;
	@FXML
	private Button btnSaveRegular;
	@FXML
	private Button btnBack;

	@FXML
	private TableView<SpecialHourRow> tableSpecialHours;
	@FXML
	private TableColumn<SpecialHourRow, String> colDate;
	@FXML
	private TableColumn<SpecialHourRow, String> colOpen;
	@FXML
	private TableColumn<SpecialHourRow, String> colClose;
	@FXML
	private TableColumn<SpecialHourRow, String> colDesc;

	@FXML
	private DatePicker dpSpecialDate;
	@FXML
	private ComboBox<String> cbSpecialOpen;
	@FXML
	private ComboBox<String> cbSpecialClose;
	@FXML
	private TextField txtSpecialDesc;
	@FXML
	private Button btnAddSpecial;
	@FXML
	private Button btnDeleteSpecial;
	@FXML
	private Button btnMarkClosed;
	...
	private void updateRegularHoursUI(Opening_Hours oh) {
		...
		DayOfWeek[] customOrder = { DayOfWeek.SUNDAY, DayOfWeek.MONDAY, DayOfWeek.TUESDAY, DayOfWeek.WEDNESDAY,
	...
	private void updateSpecialHoursTable(Opening_Hours oh) {
		...
		for (Map.Entry<LocalDate, TimeRange> entry : oh.getExceptionSchedule().entrySet()) {
			...
			specialRows.add(new SpecialHourRow(entry.getKey().toString(),
	...
	/**
	 * Gathers the special hour data from the UI and sends an add request to the
	 * server. Validates input and checks for duplicate dates before sending.
	 */
		...
		try {
			...
			for (SpecialHourRow row : tableSpecialHours.getItems()) {
				if (row.getDate().equals(dateString)) {
					showAlert("Duplicate Date",
							"Error: You can only have one special schedule per date.\nPlease delete the existing entry first.");
					return;
				}
			}
	...
	/**
	 * Gathers the selected special hour from the table and sends a delete request
	 * to the server. Validates selection before sending.
	 */
	...
	/**
	 * Helper class to group controls for each day's row. Contains ComboBoxes for
	 * open/close times and a CheckBox for active status.
	 */
	...
	/**
	 * Data model for a row in the Special Hours table. Contains date, open time,
	 * close time, and description. Used for TableView display and data binding.
	 */
