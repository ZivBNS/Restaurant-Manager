...
/**
 * Unified Controller for Managing Reservations (Admin View).
 * Handles table display, manual creation, and administrative updates.
 * Styled with a dark theme to match the AddReservation screen.
 */
public class ManageOrders_GUI {
    ...
    // --- Form Components ---
    @FXML private TextField txtOrderID, txtUserID, txtPhone, txtEmail, txtTable;
    ...
    // Buttons
    @FXML private Button btnAdd; // Added for disabling during edit mode
    ...
    /**
     * Initializes the controller. Sets up table columns, spinner, 
     * status dropdown, and dark-themed component styling.
     */
    @FXML
    ...
    /**
     * Applies dark mode styling to the Time ComboBox cells to match AddReservation screen.
     */
    ...
    private void populateForm(Reservation res) {
        ...
        if(status != null) cbStatus.setValue(status.substring(0, 1).toUpperCase() + status.substring(1).toLowerCase());
    ...
    /**
     * Toggles interaction buttons based on selection.
     * Disables the "Create" button if an order is being updated.
     */
    ...
    void onDeleteClicked(ActionEvent event) {
        ...
        if (selected == null) return;
        Alert alert = new Alert(Alert.AlertType.CONFIRMATION, "Cancel Reservation #" + selected.getConfirmationCode() + "?", ButtonType.YES, ButtonType.NO);
    ...
    /**
     * Logic to create a Reservation object from form fields, 
     * adjusting for shifts crossing midnight.
     */
    private Reservation buildReservationFromForm() {
        ...
        return new Reservation(activeInternalUserId, txtPhone.getText().trim(), txtEmail.getText().trim(), start, start.plusHours(2), spGuests.getValue());
    }
        ...
        Platform.runLater(new Runnable() {
            ...
            public void run() {
                ...
                if (oh.getExceptionSchedule().containsKey(selectedDate)) {
                    ...
                    if (range.isActive()) { open = range.getOpenTime(); close = range.getCloseTime(); }
                } else {
                ...
                if (open != null && close != null) {
                    ...
                    if (selectedDate.equals(LocalDate.now())) {
                        ...
                        if (isMidnightCrossing ? (buffer.isAfter(open) || buffer.isBefore(close)) : buffer.isAfter(open)) first = buffer;
                    }

                    LocalTime curr = first;
                    int safety = 0;
                    while (safety < 48) {
                        boolean valid = isMidnightCrossing ? (!curr.isBefore(open) || !curr.isAfter(last)) : (!curr.isBefore(open) && !curr.isAfter(last));
                        if (valid) timeCombo.getItems().add(String.format("%02d:%02d", curr.getHour(), curr.getMinute()));
                        else break;
                        if (curr.equals(last)) break;
                        curr = curr.plusMinutes(30); safety++;
                    }
                    if (timeToSelect != null) timeCombo.setValue(timeToSelect);
                }
            }
        });
    }
    ...
    @FXML public void refreshAdminData() { ConnectToServer_GUI.clientController.sendGetAllPendingReservationsRequest(); }
    
    @FXML void onCreateBillClicked(ActionEvent event) {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/gui/BillManager.fxml"));
            Stage s = new Stage(); s.setTitle("Bill Manager"); s.initModality(Modality.WINDOW_MODAL);
            s.initOwner(adminTable.getScene().getWindow()); s.setScene(new Scene(loader.load())); s.showAndWait();
            refreshAdminData();
    ...
    public void updateAdminUI(final List<Reservation> list) {
        Platform.runLater(new Runnable() {
            @Override public void run() { masterData.setAll(list); }
        });
    }

    private void showAlert(final String title, final String content) {
        Platform.runLater(new Runnable() {
            @Override public void run() { new Alert(Alert.AlertType.WARNING, content).showAndWait(); }
        });
    }
    ...
    public void showNoTableAlert(final LocalDateTime suggested) {
        Platform.runLater(new Runnable() {
            @Override public void run() {
                if (suggested == null) { new Alert(Alert.AlertType.ERROR, "Fully Booked").show(); return; }
                String msg = "Full. Nearest: " + suggested.format(DateTimeFormatter.ofPattern("dd/MM HH:mm")) + ". Update form?";
                Alert alert = new Alert(Alert.AlertType.CONFIRMATION, msg);
                alert.showAndWait().ifPresent(new java.util.function.Consumer<ButtonType>() {
                    @Override public void accept(ButtonType response) {
